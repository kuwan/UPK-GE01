...
class StrengthenTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let lable_duration = UILabel.init(frame: CGRect(x: 10*lengthPercent, y: 100*HeighPercent, width: screenWidth - 60*lengthPercent, height: 20))
        lable_duration.text = "Duration: 20 min"
        cell.contentView.addSubview(lable_duration)
        
        let lable_myrating = UILabel.init(frame: CGRect(x: 10*lengthPercent, y: 120*HeighPercent, width: self.view.frame.width - 60*lengthPercent, height: 20*lengthPercent))
